// lib/roadm/db/slot_state_data_tbl.js
 ...
 */
slotStateData.getData = function(param) {
    return new Promise((function(resolve, reject) {
        ...
        pool.getConnection(function(err, conn) {
            ...
            conn.query(
                slotStateData.makeSelectQuery(param),
                function(err, rows, field) {
                    ...
                    let resArray = [];
                    rows.forEach((row) => {
                        const obj = slotStateData.parseSlotStateData(row);
                        resArray.push(obj);
                    });
 ...
 */
slotStateData.makeSelectQuery = function(param) {
    let query = `\
select a.NE_TYPE, a.ELEMENT_NAME, b.* \
from ELEMENT_TBL a, ROADM_SLOT_STATE_DATA_TBL b`;
    let where = 'a.ELEMENT_ID = b.ELEMENT_ID and a.SEQ_NO = b.SEQ_NO';
    if (param.element_id != undefined) {
        if (where.length > 0) {
            where += ' and ';
        }
        where += `b.ELEMENT_ID = ${param.element_id}`;
    }
    if (param.seq_no != undefined) {
        if (where.length > 0) {
            where += ' and ';
        }
        where += `b.SEQ_NO = ${param.seq_no}`;
    }
    if (param.shelf_id != undefined) {
        if (where.length > 0) {
            where += ' and ';
        }
        where += `b.SHELF_ID = ${param.shelf_id}`;
    }
    if (param.slot_id != undefined) {
        if (where.length > 0) {
            where += ' and ';
        }
        where += `b.SLOT_ID = ${param.slot_id}`;
    }
    if (where.length > 0) {
        query += ' where ' + where;
    }
 ...
 */
slotStateData.parseSlotStateData = function(row) {
    let provData = null;
    const buffer = row.SYS_PROV_DATA;
    const sysId = util.HIBYTE(row.NE_TYPE);
        ...
        case ROADM_COMMON.SYSID.MPUTYPE_WCU:
        {
            switch (row.PROV_BID) {
                case ROADM_V4.WCS.BID.WCU:
                    {
                        const decodeName = iconv.decode(
                            buffer.slice(12, 32), 'euckr').toString();
                        const decodePtlm = [
                            iconv.decode(
                                buffer.slice(33, 45), 'euckr').toString(),
                            iconv.decode(
                                buffer.slice(46, 58), 'euckr').toString(),
                            iconv.decode(
                                buffer.slice(59, 71), 'euckr').toString(),
                            iconv.decode(
                                buffer.slice(72, 84), 'euckr').toString(),
                        ];
                        provData = {
                            bd_act: buffer[2],
                            ver: buffer.readUInt16BE(3),
                            time: ROADM_COMMON.parseTimeObject(buffer, 5),
                            name: decodeName.toString(),
                            ptlm: [
                                decodePtlm[0].toString(),
                                decodePtlm[1].toString(),
                                decodePtlm[2].toString(),
                                decodePtlm[3].toString(),
                            ],
                        };
                    }
                    break;
                ...
                default:
                logger.warn(`\
unknown SLOT(${row.SLOT_ID}), BID(${row.PROV_BID})`);
                    break;
            }
        }
        break;
        ...
        case ROADM_COMMON.SYSID.MPUTYPE_CCU:
        {
            switch (row.PROV_BID) {
                case ROADM_V4.CCS.BID.CCU:
                    {
                        const decodeName = iconv.decode(
                            buffer.slice(3, 23), 'euckr').toString();
                        provData = {
                            bd_act: buffer[2],
                            name: decodeName.toString(),
                            clk: {
                ...
                case U8100_V1.BID.WS_OCPMA_8F:
                    {
                        ...
                        for (let i = 0; i < 16; i++) {
                            provData.p_act.push(buffer[4 + i]);
                            provData.p_degree.push(buffer[20 + i]);
                        }
                    }
                    break;
                ...
                case U8100_V1.BID.WS_OLPMA_16:
                    {
                        ...
                        for (let i = 0; i < 16; i++) {
                            provData.p_act.push(buffer[4 + i]);
                            provData.distance.push(buffer[20 + i]);
                        }
                    }
                    break;
                ...
                case U8100_V1.BID.WS_OLCA:
                    {
                        ...
                        for (let i = 0; i < 4; i++) {
                            provData.p_act.push(buffer[4 + i]);
                            provData.p_degree.push(buffer[8 + i]);
                            provData.p_csid.push(buffer[12 + i]);
                            provData.p_cuid.push(buffer[16 + i]);
                            provData.p_cnpo.push(buffer[20 + i]);
                        }
                    }
                    break;
...
unknown SLOT(${row.SLOT_ID}), BID(${row.PROV_BID})`);
    ...
    const obj = {
        element_name: decodeData.toString(),
